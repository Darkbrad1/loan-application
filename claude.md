...
- **Some dropdowns always use the form's own options (`force`),** because the form decides what's allowed: role (no "Primary Applicant"), liability type (LiabilityType records), expense type (filtered by loan category), the responsible applicant, and the owner in ownership splits.
